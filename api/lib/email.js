...
export async function sendContactConfirmationEmail(contactData) {
  const { name, email, payment_proof_submitted } = contactData;

  const resendApiKey = process.env.RESEND_API_KEY;
  const businessEmail = process.env.BUSINESS_EMAIL || 'huascaran360mtb@gmail.com';

  const subject = 'Confirmación de inscripción - Huascarán 360 MTB';
  const participantName = name || 'Participante';

  const paymentProofLine = payment_proof_submitted
    ? 'Hemos recibido tu comprobante de pago y lo revisaremos.'
    : 'Si deseas, puedes enviar tu comprobante de pago respondiendo a este correo.';

  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #d92532; margin-bottom: 12px;">Inscripción recibida</h2>
      <p style="margin: 0 0 16px;">Hola ${participantName},</p>
      <p style="margin: 0 0 16px;">
        ¡Gracias! Hemos recibido tu inscripción para Huascarán 360 MTB.
      </p>
      <div style="background: #f8fafc; padding: 16px; border-radius: 10px; border: 1px solid #e2e8f0;">
        <p style="margin: 0 0 8px;"><strong>Email registrado:</strong> ${email}</p>
        <p style="margin: 0;">${paymentProofLine}</p>
      </div>
      <p style="margin: 16px 0 0;">
        En breve un miembro del equipo se pondrá en contacto contigo. Si no ves este correo en tu bandeja de entrada,
        revisa la carpeta de spam.
      </p>
      <p style="margin: 16px 0 0; color: #64748b; font-size: 14px;">
        Si tienes dudas, responde a este correo o escribe a <a href="mailto:${businessEmail}">${businessEmail}</a>.
      </p>
    </div>
  `;

  const textContent = `
Inscripción recibida - Huascarán 360 MTB

Hola ${participantName},

¡Gracias! Hemos recibido tu inscripción.

Email registrado: ${email}
${paymentProofLine}

En breve un miembro del equipo se pondrá en contacto contigo.
Si no ves este correo, revisa la carpeta de spam.

Contacto: ${businessEmail}
  `.trim();

  if (!resendApiKey) {
    console.log('=== CONTACT CONFIRMATION (Email not configured) ===');
    console.log('RESEND_API_KEY is not set in environment variables');
